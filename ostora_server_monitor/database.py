...
if __name__ == "__main__":
    ...
    from ostora_server_monitor.async_utils import run_in_executor
...
class Database:
    ...
    def connect(self):
        ...
        if (
            ...
            or DB_CONNECTION == Driver.PostgreSQL.value
        ):
            ...
            self.database = os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "..", "data", "servers.db"
            )
    ...
    def count_servers_per_game(self):
        if self.driver == Driver.MongoDB:
            pipeline = [{"$group": {"_id": "$game_id", "count": {"$sum": 1}}}]
            results = self.servers.aggregate(pipeline)
            servers_count = {str(row["_id"]): int(row["count"]) for row in results}
            results.close()
            return servers_count

        conn, cursor = self.cursor()
        cursor.execute(
            self.transform("SELECT game_id, COUNT(*) FROM servers GROUP BY game_id")
        )
    ...
    def __all_servers(
        ...
        *,
        ...
        if self.driver == Driver.MongoDB:
            if channel_id:
                results = self.servers.find({"channel_id": channel_id}).sort("position")
            elif guild_id:
                results = self.servers.find({"guild_id": guild_id}).sort("position")
            elif message_id:
                results = self.servers.find({"message_id": message_id}).sort("position")
            elif game_id:
                results = self.servers.find({"game_id": game_id}).sort("position")
            else:
        ...
        if channel_id:
            cursor.execute(
                self.transform(
                    "SELECT * FROM servers WHERE channel_id = ? ORDER BY position"
                ),
                (channel_id,),
            )
        elif guild_id:
            cursor.execute(
                self.transform(
                    "SELECT * FROM servers WHERE guild_id = ? ORDER BY position"
                ),
                (guild_id,),
            )
        elif message_id:
            cursor.execute(
                self.transform(
                    "SELECT * FROM servers WHERE message_id = ? ORDER BY position"
                ),
                (message_id,),
            )
        elif game_id:
            cursor.execute(
                self.transform("SELECT * FROM servers WHERE game_id = ? ORDER BY id"),
                (game_id,),
            )
        else:
    ...
    def add_server(self, s: Server):
        if self.driver == Driver.MongoDB:
            try:
                max_position = self.servers.find_one(
                    {"channel_id": s.channel_id}, sort=[("position", -1)]
                )["position"]
            except TypeError:
                max_position = 0
        ...
        cursor.execute(
            ...
            (
                ...
                s.address,
                s.query_port,
                stringify(s.query_extra),
                s.status,
                stringify(s.result),
                s.style_id,
                stringify(s.style_data),
            ),
        )
        self.close(conn, cursor, commit=True)
    ...
    def update_servers_message_id(self, servers: list[Server]):
        if self.driver == Driver.MongoDB:
            operations = [
                UpdateOne(
                    {"_id": server.id}, {"$set": {"message_id": server.message_id}}
                )
    ...
    def update_servers(self, servers: list[Server], *, channel_id: int = None):
        ...
        parameters = [
            (server.status, stringify(server.result), server.id) for server in servers
        ]
        sql = "UPDATE servers SET status = ?, result = ? WHERE id = ?"
        conn, cursor = self.cursor()
        cursor.executemany(self.transform(sql), parameters)
        self.close(conn, cursor, commit=True)
    ...
    def find_server(self, channel_id: int, address: str = None, query_port: int = None):
        return self.__find_server(
            channel_id=channel_id, address=address, query_port=query_port
        )

    def __find_server(
        self, channel_id: int, address: str = None, query_port: int = None
    ):
        if self.driver == Driver.MongoDB:
    ...
    def modify_server_position(self, server1: Server, direction: bool):
        ...
        if (
            len(indices) <= 0
            or (direction and indices[0] == 0)
            or (not direction and indices[0] == len(servers) - 1)
        ):
            return []
    ...
    def __swap_servers_positon(self, server1: Server, server2: Server):
        if self.driver == Driver.MongoDB:
            ...
            self.servers.update_one(
                ...
                {
                    "$set": {
                        "position": server1.position,
                        "message_id": server1.message_id,
                    }
                },
            )
        else:
    ...
    def update_servers_style_data(self, servers: list[Server]):
        if self.driver == Driver.MongoDB:
            if operations := [
                UpdateOne(
                    {"_id": server.id}, {"$set": {"style_data": server.style_data}}
                )
                for server in servers
            ]:
                self.servers.bulk_write(operations, ordered=False)
    ...
    def __update_servers_channel_id(self, servers: list[Server], channel_id: int):
        ...
        parameters = [(channel_id, channel_id, server.id) for server in servers]
        conn, cursor = self.cursor()
        cursor.executemany(self.transform(sql), parameters)
        self.close(conn, cursor, commit=True)
    ...
    def export(self, *, to_driver: str):
        ...
        export_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "..", "data", "exports"
        )
    ...
    def import_(self, *, filename: str):
        ...
        if filename.endswith(".json"):
            ...
            if self.driver != Driver.MongoDB:
                raise ValueError("Invalid driver for JSON file. Expected 'mongodb'.")
...
if __name__ == "__main__":
    ...
    export.add_argument("--to_driver", choices=drivers, default=database.driver.value)
