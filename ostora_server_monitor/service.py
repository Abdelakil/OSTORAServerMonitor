...
from ostora_server_monitor.database import Database
from ostora_server_monitor.gamedig import Gamedig
...
whitelist_guilds = (
    ...
    if public
        ...
        for guild in os.getenv("WHITELIST_GUILDS", "").replace(";", ",").split(",")
        if guild
    ]
)
...
if public:
    sponsors_file = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "..", "sponsors.json"
    )

    if not os.path.isfile(sponsors_file):
        with open(sponsors_file, "w", encoding="utf8") as f:
            json.dump(
                {"348921660361146380": {"id": "OTg1OTU1Nzg0NTE1MDE4ODQy", "limit": 30}},
                f,
                indent=4,
            )
