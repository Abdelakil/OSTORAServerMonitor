...
class Teamspeak3(Protocol):
    ...
    async def query(self):
        if "teamspeakQueryPort" in self.kv:  # backward compatibility
            host, port, voice_port = (
                str(self.kv["host"]),
                int(str(self.kv["teamspeakQueryPort"])),
            ...
            host, port, voice_port = (
                str(self.kv["host"]),
                int(str(self.kv["port"])),
                int(str(self.kv["voice_port"])),
            )

        teamspeak3 = opengsq.TeamSpeak3(host, port, voice_port, self.timeout)
        start = time.time()
        info, clients, channels = await asyncio.gather(
            teamspeak3.get_info(), teamspeak3.get_clients(), teamspeak3.get_channels()
        )
        ping = int((time.time() - start) * 1000)
        players = [
            {"name": player["client_nickname"], "raw": player}
            for player in clients
            if player.get("client_type") == "0"
        ]
        ...
        result: GamedigResult = {
            ...
            "ping": ping,
            "raw": {"info": info, "channels": channels},
        }
